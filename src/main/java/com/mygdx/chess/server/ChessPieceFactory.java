...
class ChessPieceFactory {
    ...
    private List<ChessPiece> listOfPawns(ChessPieceColor color) {
        ...
        for (int i = 0; i < 8; i++) {
            chessPawns.add(createChessPiece(new ChessPieceDto(PAWN, color, i, color.getYPawnsPosition()), false));
        }
        return chessPawns;
    }
    ...
    private List<ChessPiece> listOfFigures(ChessPieceColor color) {
        List<ChessPiece> chessPieces = new ArrayList<>();
        ChessPieceType[] types = {ROOK, KNIGHT, RUNNER, QUEEN, KING, RUNNER, KNIGHT, ROOK};
        for (int i = 0; i < types.length; i++) {
            chessPieces.add(
                createChessPiece(new ChessPieceDto(types[i], color, i, color.getYFiguresPosition()), false));
        }
        return chessPieces;
    }
